...
create_env_files() {
    ...
    # Backend environment
    cat > ./multi-agent-backend/.env << EOF
...
EOF
    ...
    cat > ./multi-agent-frontend/.env.local << EOF
...
EOF
    ...
    # Agents environment
    cat > ./multi-agent-agents/.env << EOF
# Agent Configuration
...
EOF
    ...
    print_warning "Please update the OPENAI_API_KEY in ./multi-agent-agents/.env"
}
